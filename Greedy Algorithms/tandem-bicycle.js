...
function tandemBicycle(redShirtSpeeds, blueShirtSpeeds, fastest) {
    ...
    if(fastest) {
        let red = redShirtSpeeds.length - 1;
        let blue = blueShirtSpeeds.length - 1;
        let max = [];
        while (red >= 0 && blue >= 0 && max.length < redShirtSpeeds.length) {
            if (redShirtSpeeds[red] >= blueShirtSpeeds[blue]) {
                max.push(redShirtSpeeds[red])
                red -= 1;
            } else if (redShirtSpeeds[red] < blueShirtSpeeds[blue]) {
                max.push(blueShirtSpeeds[blue]);
                blue -= 1;
            }
        }
        ...
        let min = [];
        while(red < redShirtSpeeds.length && blue<blueShirtSpeeds.length && min.length < redShirtSpeeds.length) {
            if (redShirtSpeeds[red] <= blueShirtSpeeds[blue]) {

                min.push(blueShirtSpeeds[blue])
                blue += 1;
                red += 1;
            } else if (redShirtSpeeds[red] > blueShirtSpeeds[blue]) {

                min.push(redShirtSpeeds[red]);
                red += 1;
                blue += 1;
            }
        }
...
//test
let {redShirtSpeeds, blueShirtSpeeds, fastest} = {
    "redShirtSpeeds": [5, 5, 3, 9, 2],
    "blueShirtSpeeds": [3, 6, 7, 2, 1],
    "fastest": false
}
